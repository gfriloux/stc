...
| `stc.cogitator.vm.linger` | bool | `true` | Keep the user's systemd instance/services running without an open session. Set `false` on hardened hosts. |
...
- Creates a user (`username`) with:
  ...
  - `shell = pkgs.fish`
  - `linger = cfg.linger` (defaults to `true`)
