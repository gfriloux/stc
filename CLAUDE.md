...
- **Plan first.** Every change is planned before code, following `PROCEDURE_PLANS.md`.
  Plans live under `.claude/plans/` (`vX.Y.Z/` per release, `release/` for tooling),
  never at the repo root. See `.claude/plans/README.md`.
- **Hybrid git.** Work on a dedicated branch (`feat/…`, `fix/…`, `refactor/…`,
  `docs/…`, `chore/…`, `ci/…`), never on `main`. Commit atomically (Conventional
  Commits). **Never** merge, push, or tag — the user does that.
- **Releases.** SemVer, pre-1.0 (`0.x`). `CHANGELOG.md` is generated by `git-cliff`
